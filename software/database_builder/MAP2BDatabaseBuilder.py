...
The serialized MAP2B database layout is:
  <enzyme>.species.marisa<boundary>
  <enzyme>.species.uniq.marisa
  <enzyme>.species.uniq.stat.xls
  abfh_classify_with_speciename.txt.gz

The builder keeps the historical MAP2B 8-digit internal genome ID plus a copy-number
field zero-padded to at least 4 digits. The final *.species.uniq.stat.xls generated by
this builder uses the 10-column expanded-taxonomy layout:
  ID, kingdom, phylum, class, order, family, genus, species,
  assembly_unique_tag_count, species_average_unique_tag_count

For the supplied/current MAP2B 1.8 runtime, BsaXI (3), BcgI (5), and CjePI (13)
are directly supported by MAP2B.py. Other Type IIB enzymes can be pre-built by
this builder but require corresponding downstream MAP2B support before use.
"""
...
def write_manifest(
    ...
    manifest = {
        ...
        "direct_map2b_1_8": enzyme.enzyme_id in DIRECT_MAP2B_ENZYME_IDS,
...
def command_validate(args: argparse.Namespace) -> None:
    ...
    if mapping_path.exists():
        ...
        warn(f"{ID_MAP_FILENAME} is missing; MAP2B can still read the database, but update auditability is reduced")
...
def print_enzyme_table() -> None:
    print("ID\tEnzyme\tTag_length\tMAP2B_1.8_direct")
    for enzyme in ENZYMES:
        direct = "yes" if enzyme.enzyme_id in DIRECT_MAP2B_ENZYME_IDS else "builder-only"
        print(f"{enzyme.enzyme_id}\t{enzyme.name}\t{enzyme.length}\t{direct}")
    print(f"{ALL_ENZYME_ID}\tAllEnzyme\t-\tbuild/validate all 16 separately")
