...
############################################ ___ #################################################
__doc__ = 'MAP2B-Cross-domain: metagenomics analysis with microbial/macro-organism abundance split. Based on MAP2B v1.8 (Sun et al.) with BsaXI format fixes and cross-domain output.'
__author__ = 'Zheng Sun, Liu Jiang'
__mail__ = 'spzsu@channing.harvard.edu, jiang.liu@oebiotech.com'
__date__ = '2026/01/06 10:03:47'
__version__ = '1.8-CrossDomain'
############################################ main ##################################################
...
#	parser.add_argument('-d',help='Database path for MAP2B pipeline, default {}'.format(def_db_dir),dest='database',type=str,default=def_db_dir)
	...
	parser.add_argument('-b',help='Keep the total abundance table (Abundance.xls). Default: only output the split tables (Abundance.micro.xls / Abundance.macro.xls). micro = Bacteria+Archaea+Fungi; macro = the rest',dest='keep_total',action='store_true')
...
#	enzyme_id = 13
	...
	if os.path.exists(done_file):
		...
		exe_shell('python3 {src_dir}/split_abundance.py -i {O}/Abundance.xls -o {O} {keep}'.format(src_dir=src_dir, O=O, keep='--keep-total' if args.keep_total else ''), 'SplitAbundance_MicroMacro')
