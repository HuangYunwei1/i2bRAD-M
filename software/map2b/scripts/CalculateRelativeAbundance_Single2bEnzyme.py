...
# reading stat file
	...
	with open(stat_file, 'r') as IN:
		for line in IN:
			...
			tmp = line.split('\t')
			if len(tmp) == 5:
				# 5-column legacy format: taxonomy is comma-separated in column 2.
				tax = '\t'.join(tmp[1].split(','))
				gcf_theo_tag_num = tmp[2]
				tax_theo_tag_num = tmp[3]
			elif len(tmp) in (10, 11):
				# 10/11-column expanded-taxonomy format:
				# columns 2-8 = kingdom..species; columns 9-10 = theoretical tag counts.
				tax = '\t'.join(tmp[1:8])
				gcf_theo_tag_num = tmp[8]
				tax_theo_tag_num = tmp[9]
			else:
				report('ERROR', 'Unsupported stat file format: {} columns'.format(len(tmp)))
			gcf_theo_tag_num_dic.setdefault(tmp[0], gcf_theo_tag_num)
			tax_theo_tag_num_dic.setdefault(tax, tax_theo_tag_num)
